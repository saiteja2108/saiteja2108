...
while True:  # main game loop
    ...
    if flag == 3:
        ...
        for coin1 in coin_array:
            for coin2 in coin_list:
                if coin1 is not coin2 and pygame.sprite.collide_circle(coin1,
                                                                       coin2) and not coin1.collided and not coin2.collided:
    ...
    # if all the coins have come to rest
    if boardHalt and flag == 3:
        ...
        # if the queen gone and then another coin not gone

        # if foul happened then we had deleted the striker
        # now again add it to the coin_array
